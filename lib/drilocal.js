...
var g = {
	...
	array_push: function array_push(a,b){
		if( b.es_constructor(a,Array) ){
			...
			g.apilar(`${arguments.calle.name} Error: El valor '${a}' no es un array.`)
		}
		return a
	}
	, substr: function substr(s,c){
		...
		if( b.es_constructor(s,String) ){
			...
			g.apilar(`esu Error: El parámetro '${s}' no es un string.`)
		}
		return d
	}
	...
	, json_decode: function json_decode(s){
		...
		try{
			...
			g.apilar(`jos Error: El JSON. '${s}' no se pudo analizar.`)
		}
		return d
	}
}
...
var ls = {
	...
	, borrar_archivo: function borrar_archivo(url){
		...
		if(bin_url_inicio_barra){
			g.apilar(`abia1 Advertencia: El archivo '${url}' es del sistema, por eso no se va a borrar.`)
		}else{
			var bin_existe_archivo = ls.file_exists(url)
			if(bin_existe_archivo){
				var bin_borrar_archivo = ls.unlink(url)
				if(bin_borrar_archivo){
					g.apilar(`bia3 Archivo '${url}' borrado correctamente.`)
				}else{
					g.apilar(`ebia5 Error: El archivo o carpeta '${url}' existe, pero no se pudo borrar.`)
				}
			}else{
				g.apilar(`abia4 Advertencia: La ruta '${url}' no contiene información, por eso no se va a borrar su contenido.`)
			}
		}
	}
	, borrar_carpeta: function borrar_carpeta(url){
		var bin_url_inicio_barra = ls.substr(url,[0,1])== "/"
		if(bin_url_inicio_barra){
			g.apilar(`ebca1 Error: La carpeta '${url}' es del sistema, por eso no se va a borrar.`)
		}else{
			var bin_borrar_carpeta = ls.rmdir(url)
			if(bin_borrar_carpeta){
				g.apilar(`bca3 Carpeta '${url}' borrada correctamente.`)
			}else{
				g.apilar(`abca2 Advertencia: La carpeta '${url}' no se pudo borrar.`)
			}
		}
	}
	...
	, crear_carpeta: function crear_carpeta(url){
		...
		if(!bin_existe_url){
			...
			if(bin_crear_carpeta){
				g.apilar(`de3 Carpeta '${url}' creada correctamente.`)
			}else{
				g.apilar(`ade2 Advertencia: La carpeta '${url}' no se pudo crear.`)
			}
		}else{
			g.apilar(`ade0 Advertencia: La carpeta '${url}' existía.`)
		}
	}
	...
	, crear_archivo: function crear_archivo(url,datos){
		...
		if(var_crear_archivo===ls.strlen(datos)){
			g.apilar(`co1 Archivo '${url}' creado correctamente.`)
		}else{
			if(var_crear_archivo===false){
				g.apilar(`aco3 Advertencia: El archivo '${url}' no se pudo crear.`)
			}else{
				g.apilar(`aco2 Advertencia: El archivo '${url}' se pudo crear, pero su contenido es parcial.`)
			}
		}
	}
	, crear_carpeta_si_no_existe: function crear_carpeta_si_no_existe(url){
		...
		if(bin_existe_url){
			var bin_es_carpeta = ls.is_dir(url)
			if(bin_es_carpeta){
				g.apilar(`cas La carpeta '${url}' existía.`)
				d = 3
			}else{
				ls.borrar_url(url)
	...
	, crear_archivo_si_no_existe: function crear_archivo_si_no_existe(url,datos){
		...
		if(bin_existe_url){
			...
			if(bin_es_carpeta){
				...
				g.apilar(`dus El archivo '${url}' existía.`)
				d = 2
			}
		}else{
			ls.crear_archivo(url,datos)
...
document.onreadystatechange = function(x){
	if(!window.t){
		...
		var nombre_documento = window.nombre_capitalizado
		b.agregar_favicon()
